...
// Mock data
const STATS = [
  { label: "총 대화 수", value: "12,847", change: "+12.5%", icon: "💬", color: "bg-blue-50 text-[#2563EB]" },
  { label: "평균 응답 시간", value: "0.3초", change: "-18.2%", icon: "⚡", color: "bg-green-50 text-green-600" },
  { label: "고객 만족도", value: "98.2%", change: "+2.1%", icon: "😊", color: "bg-yellow-50 text-yellow-600" },
  { label: "활성 채팅", value: "24", change: "+5", icon: "🔥", color: "bg-red-50 text-red-500" },
];

const RECENT_CONVERSATIONS = [
  { id: 1, user: "김서연", avatar: "KS", message: "주문한 상품의 배송 상태를 확인하고 싶어요", time: "2분 전", status: "진행중", satisfaction: null },
  { id: 2, user: "이준호", avatar: "LJ", message: "반품 절차가 어떻게 되나요?", time: "15분 전", status: "완료", satisfaction: 5 },
  { id: 3, user: "박민지", avatar: "PM", message: "쿠폰 코드가 작동하지 않습니다", time: "32분 전", status: "완료", satisfaction: 4 },
  { id: 4, user: "정하늘", avatar: "JH", message: "회원가입 시 혜택이 뭔가요?", time: "1시간 전", status: "완료", satisfaction: 5 },
  { id: 5, user: "최우진", avatar: "CW", message: "결제 시 오류가 발생합니다", time: "1시간 전", status: "상담원 전환", satisfaction: 3 },
  { id: 6, user: "강예진", avatar: "KY", message: "포인트 적립 기준이 궁금합니다", time: "2시간 전", status: "완료", satisfaction: 5 },
  { id: 7, user: "윤도현", avatar: "YD", message: "주문 취소는 어떻게 하나요?", time: "3시간 전", status: "완료", satisfaction: 4 },
];
...
function Sidebar({ activeTab, setActiveTab }: { activeTab: TabType; setActiveTab: (t: TabType) => void }) {
            ...
            <div key={conv.id} className="px-6 py-3.5 flex items-center gap-4 hover:bg-gray-50/50 transition-colors cursor-pointer">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-gray-900">{conv.user}</span>
                ...
                <p className="text-xs text-gray-500 truncate mt-0.5">{conv.message}</p>
              </div>
                ...
                {conv.satisfaction && (
                  <div className="flex gap-0.5 mt-1 justify-end">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <span key={i} className={`text-[10px] ${i < conv.satisfaction! ? "text-yellow-400" : "text-gray-200"}`}>
                        ★
                      </span>
                    ))}
                  </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <input
            type="text"
            placeholder="검색..."
            className="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#2563EB]/30 focus:border-[#2563EB]/30"
          />
          ...
          <tbody className="divide-y divide-gray-50">
            {RECENT_CONVERSATIONS.map((conv) => (
              <tr key={conv.id} className="hover:bg-gray-50/50 cursor-pointer transition-colors">
                ...
                <td className="px-6 py-3.5 text-sm text-gray-500 max-w-xs truncate">{conv.message}</td>
                <td className="px-6 py-3.5">
                ...
                <td className="px-6 py-3.5">
                  {conv.satisfaction ? (
                    <div className="flex gap-0.5">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <span key={i} className={`text-sm ${i < conv.satisfaction! ? "text-yellow-400" : "text-gray-200"}`}>
                          ★
                        </span>
                      ))}
                    </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-5 rounded-xl border border-gray-100">
          ...
          <div className="text-3xl font-bold text-gray-900">12.7%</div>
          <div className="mt-2 w-full h-2 bg-gray-100 rounded-full overflow-hidden">
        ...
        <div className="bg-white p-5 rounded-xl border border-gray-100">
          ...
          <div className="text-3xl font-bold text-gray-900">4.2회</div>
          <div className="mt-2 w-full h-2 bg-gray-100 rounded-full overflow-hidden">
